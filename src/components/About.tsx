    ...
    <section id="about" className="py-20 bg-white" ref={sectionRef}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16 animate-on-scroll opacity-0 translate-y-10 transition-all duration-1000">
          <h2 className="text-4xl font-bold text-gray-900 mb-4 glowing-text">
            About Me
          </h2>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {skills.map((skill, index) => (
            <div key={index} className="group perspective">
              <div
                className={`p-6 bg-gray-50 rounded-lg animate-on-scroll opacity-0 translate-y-10 transition-all duration-1000 hover:shadow-xl transform-gpu group-hover:rotate-y-10 group-hover:scale-105 card-3d`}
                style={{ transitionDelay: `${index * 200}ms` }}
              >
                <div className="w-12 h-12 bg-blue-100 rounded-lg flex items-center justify-center text-blue-600 mb-4 transform transition-all duration-500 group-hover:rotate-y-180 group-hover:scale-110">
                  {skill.icon}
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {skill.technologies.map((tech, techIndex) => (
                    <span
                      key={techIndex}
                      className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-sm hover:bg-blue-200 transition-all duration-300 transform hover:scale-105 hover:rotate-3"
                    >
                      {tech}
                    </span>
                  ))}
