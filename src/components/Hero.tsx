import { useEffect, useState } from 'react';
import { ArrowRight } from 'lucide-react';

const Hero = () => {
  const [isVisible, setIsVisible] = useState(false);
  const [mousePosition, setMousePosition] = useState({ x: 0, y: 0 });

  useEffect(() => {
    setIsVisible(true);

    const handleMouseMove = (e: MouseEvent) => {
      const x = (e.clientX / window.innerWidth - 0.5) * 20;
      const y = (e.clientY / window.innerHeight - 0.5) * 20;
      setMousePosition({ x, y });
    };

    window.addEventListener('mousemove', handleMouseMove);
    return () => window.removeEventListener('mousemove', handleMouseMove);
  }, []);

  return (
    <section id="home" className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-50 pt-16 overflow-hidden perspective">
      <div 
        className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 transform-3d"
        style={{
          transform: `rotateX(${mousePosition.y * 0.1}deg) rotateY(${mousePosition.x * 0.1}deg)`
        }}
      >
        <div className="text-center relative">
          <div className="absolute inset-0 grid grid-cols-8 grid-rows-8 gap-1 pointer-events-none">
            {Array.from({ length: 64 }).map((_, i) => (
              <div
                key={i}
                className="bg-blue-200/10 rounded-lg transform transition-transform duration-700"
                style={{
                  transform: `translateZ(${Math.sin((i + mousePosition.x) * 0.1) * 20}px)`,
                }}
              />
            ))}
          </div>
          <h1 
            className={`text-5xl md:text-7xl font-bold text-gray-900 mb-6 transition-all duration-1000 transform glowing-text ${
              isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
            }`}
          >
            Full Stack Developer
          </h1>
          <p 
            className={`text-xl md:text-2xl text-gray-600 mb-8 max-w-2xl mx-auto transition-all duration-1000 delay-300 transform ${
              isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
            }`}
          >
            I craft beautiful, scalable web applications with modern technologies
          </p>
          <div 
            className={`flex justify-center gap-4 transition-all duration-1000 delay-500 transform ${
              isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
            }`}
          >
            <a
              href="#projects"
              className="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 hover:scale-105 transform transition-all duration-300 shadow-neon"
            >
              View My Work
              <ArrowRight className="ml-2 w-5 h-5 animate-float" />
            </a>
            <a
              href="#contact"
              className="inline-flex items-center px-6 py-3 border border-gray-300 text-base font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 hover:scale-105 transform transition-all duration-300 glass-effect"
            >
              Contact Me
            </a>
          </div>
        </div>
      </div>
    </section>
  );
};

export default Hero;